...
function initializeServiceWorker() {
  ...
  // sw.js is executed.
  const registerServiceWorekr = async () => {
  ...
  const myPromise = new Promise( async (resolve, reject) => {
    ...
    for (let index = 0; index < RECIPE_URLS.length; index++){
      ...
      try {
          ...
          const myResponse = await fetch(RECIPE_URLS[index]);
          ...
          if (index === (RECIPE_URLS.length - 1)) {
            saveRecipesToStorage(myRecipes);
            Promise.resolve(myRecipes);
          }
      } catch (err) {
